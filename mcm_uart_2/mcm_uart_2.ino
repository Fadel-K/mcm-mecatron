...
// ---------------- Pins ----------------
const uint8_t PIN_POT      = A1;   // throttle
const uint8_t PIN_DIR_SW   = 12;   // direction switch (to GND, INPUT_PULLUP)
...
// ---------------- Rate / debounce -----
const uint16_t SEND_HZ     = 2;           // send up to 20 msgs/sec
...
void loop()
{
  ...
  if (stc != last_stc || (now - lastSendMs) >= SEND_PERIOD) {
    ...
    Serial.write((uint8_t)0x00&0x00);
